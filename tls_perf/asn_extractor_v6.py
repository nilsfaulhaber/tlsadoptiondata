...
with open("tls13_tls12_SupportingUrls_v6.txt","r") as txtFile:
...
def getRow(lines):
    for line in lines:
        try:
            print(line)
            ip = socket.getaddrinfo(line, None, socket.AF_INET6)[0][4][0]
            print(ip)
...
def writeAsnDataToCsv():
    ...
    today = datetime.date.today()
    filename = "asnMappingData" + str(today) + "_v6.csv"
...
def analyzeAndPerformTest():
    today = datetime.date.today()
    filename_mapping = "asnMappingData" + str(today) + "_v6.csv"
    asns = {}
    asn_to_holder = {}
    ...
    filename = "performance_output" + str(today) + "_v6.csv"
...
if __name__ == '__main__':
    ...
    #try:
        ...
        #getRow(line)        path = OUTPUT_FOLDER + filename
