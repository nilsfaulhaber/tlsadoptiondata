...
def get_urls_to_values(mode):
    if mode == "TLS1.2":
        with open(OUTPUT_FOLDER + "tls12_performance_per_url.txt", 'r') as f:
            content = f.readlines()
            urls_to_values = {}
            for line in content:
        ...
        with open(OUTPUT_FOLDER + "tls13_performance_per_url.txt", 'r') as f:
            content = f.readlines()
            urls_to_values = {}
            for line in content:
        ...
        with open(OUTPUT_FOLDER + "conn_diff_per_url.txt", 'r') as f:
            content = f.readlines()
            urls_to_values = {}
            for line in content:
...
def write_data_to_files():
    ...
    dates = []
    with open(OUTPUT_FOLDER + 'dates.txt', 'r') as f:
        content = f.readlines()
        ...
        for index, row in dataTLS12df.iterrows():
            ...
            if not (t.empty):
                ...
                (urls_to_values_12, urls_to_values_13, urls_to_conn_diff) = write_data_for_url(t.at[0, "Url"],
                                                                                               ...
                                                                                               urls_to_values_13,
                                                                                               urls_to_conn_diff)
                write_dict_to_file(urls_to_values_12, OUTPUT_FOLDER + "tls12_performance_per_url.txt")
                write_dict_to_file(urls_to_values_13, OUTPUT_FOLDER + "tls13_performance_per_url.txt")
                write_dict_to_file(urls_to_conn_diff, OUTPUT_FOLDER + "conn_diff_per_url.txt")
        vals_sorted = np.sort(dfTls13Tls12['ConnDiff'].dropna())
        with open(OUTPUT_FOLDER + 'dates.txt', 'a') as f:
            f.write(file_date + "\n")
        with open(OUTPUT_FOLDER + 'tls13_performance_data.txt', 'a') as f:
            f.write(file_date + ";")
            for value in performance13_data:
                f.write(str(value) + ",")
            f.write("\n")
        with open(OUTPUT_FOLDER + 'tls12_performance_data.txt', 'a') as f:
            f.write(file_date + ";")
            for value in performance12_data:
                f.write(str(value) + ",")
            f.write("\n")
        with open(OUTPUT_FOLDER + 'connection_differences.txt', 'a') as f:
            f.write(file_date + ";")
